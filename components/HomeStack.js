...
export default function HomeTab() {
  ...
  // STATE: Controls whether to show all items or just 3
  ...
  // LOGIC: Show all or slice first 3
  const displayData = showAll ? allDocuments : allDocuments.slice(0, 3);

  const renderDocItem = ({ item }) => (
    <View style={[styles.card, { backgroundColor: theme.colors.card, borderLeftColor: item.urgent ? '#ff4d4d' : theme.colors.border }]}>
      ...
      <Text style={[styles.cardSummary, { color: '#888' }]} numberOfLines={2}>
        {item.summary}
      </Text>
    </View>
  );
  ...
  const renderListHeader = () => (
    <View>
      ...
      <View style={styles.sectionHeaderRow}>
        ...
        {/* BUTTON: Toggle between "View All" and "Show Less" */}
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.header, { backgroundColor: theme.colors.card, borderBottomColor: theme.colors.border }]}>
        ...
        <View style={styles.headerTextContainer}>
          ...
          <Text style={[styles.userName, { color: theme.colors.text }]}>Station Manager</Text>
        </View>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  header: {
    flexDirection: 'row', alignItems: 'center', padding: 15, borderBottomWidth: 1, paddingTop: 40,
  },
  menuButton: { padding: 5, marginRight: 15 },
  headerTextContainer: { flex: 1 },
  welcomeText: { fontSize: 12, color: '#888' },
  userName: { fontSize: 18, fontWeight: 'bold' },
  bellButton: { padding: 5, marginLeft: 10, position: 'relative' },
  notificationDot: { position: 'absolute', top: 5, right: 5, width: 8, height: 8, backgroundColor: '#ff4d4d', borderRadius: 4 },
  statsContainer: { flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 20, marginTop: 20, marginBottom: 25 },
  statCard: { flex: 1, padding: 15, borderRadius: 12, marginHorizontal: 5, justifyContent: 'center', alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  totalDocsCard: { backgroundColor: '#0056b3' },
  unreadCard: { backgroundColor: '#ff9800' },
  historyCard: { backgroundColor: '#4caf50' },
  statLabel: { color: '#fff', fontSize: 11, marginTop: 5, fontWeight: '600', textTransform: 'uppercase' },
  statNumber: { color: '#fff', fontSize: 18, fontWeight: 'bold', marginTop: 2 },
  ...
  viewAllText: { fontSize: 14, color: '#0056b3', fontWeight: '600' },
  listContent: { paddingBottom: 20 },
  card: { marginHorizontal: 20, borderRadius: 12, padding: 15, marginBottom: 15, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3, borderLeftWidth: 5 },
